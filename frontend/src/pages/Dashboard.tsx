...
export const Dashboard = (): JSX.Element => {
  ...
  return (
    <div className={`flex min-h-screen w-full ${isDarkMode ? 'bg-[#222222]' : 'bg-gray-50'}`}>
      ...
      <Button
        className={`fixed top-4 left-4 z-[100] md:hidden ${isDarkMode ? 'bg-[#333333] text-white' : 'bg-white text-gray-800'}`}
      ...
      {/* Sidebar */}
      <aside
        className={`fixed md:relative w-[300px] min-h-screen ${isDarkMode ? 'bg-[#222222] text-neutral-200' : 'bg-white text-gray-800'} p-4 flex flex-col border-r border-solid ${isDarkMode ? 'border-[#ffffff26]' : 'border-gray-200'} transition-transform duration-300 ease-in-out z-50 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'
          }`}
      >
