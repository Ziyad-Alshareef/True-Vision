...
class ForgotPasswordView(APIView):
    ...
    def post(self, request):
        ...
        print(f"Attempting to send reset PIN to email: {email}")
        
        try:
            # For testing, use a more lenient lookup
            ...
            print(f"Generated PIN: {pin} for user: {user.username}")
            
            # Send email with the PIN
            from django.core.mail import send_mail
            from django.conf import settings
            
            # Temporarily override the email backend for testing
            ...
            # Create a connection to the console backend for testing
            ...
            email_sent = False
            try:
                # First try with console backend
                ...
                print(f"Test email sent successfully to console with result: {email_sent}")
                
                # Then try with the configured backend
                email_sent = send_mail(
                    'True Vision - Password Reset PIN',
                    f'Your PIN is {pin}. It is valid for 10 minutes.',
                    settings.DEFAULT_FROM_EMAIL,
                ...
                print(f"Real email sent successfully with result: {email_sent}")
            ...
            print(f"User.DoesNotExist error for email: {email}")
            # For security reasons, don't reveal that the email doesn't exist
            return Response(
                {"message": "If your email exists in our system, you will receive a PIN."}, 
                status=status.HTTP_200_OK
            ...
            print(f"Unexpected error in ForgotPasswordView: {str(e)}")
            return Response(
                {"error": str(e)}, 
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
    ...
    def post(self, request):
        ...
        try:
            ...
            if user.is_reset_pin_valid(pin):
                print(f"PIN is valid for user: {user.username}")
                user.set_password(new_password)
                ...
                print(f"Invalid or expired PIN for user: {user.username}")
                return Response(
            ...
            print(f"Unexpected error in ResetPasswordView: {str(e)}")
            return Response(
                {"error": str(e)}, 
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
